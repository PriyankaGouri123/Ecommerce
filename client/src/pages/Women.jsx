...
export default function Women() {
  ...
  // Compute women products (empty array if not loaded yet)
  const womenProducts = (products || []).filter((p) => p.category === "Women");
  const { filteredProducts, activeFilterCount } = useProductFilter(womenProducts, filters, sortBy);
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      <div className="relative h-[520px] overflow-hidden">
        <img
          src="/photos/bannerwomen.png"
          className="w-full h-full object-cover"
          onError={(e) => e.target.src = "https://images.unsplash.com/photo-1483985988355-763728e1935b?auto=format&fit=crop&w=1600&q=80"}
        />
        <div className="absolute inset-0 bg-gradient-to-r from-black/55 to-transparent flex items-center px-12">
          <div className="max-w-7xl mx-auto w-full">
            <h1 className="text-white text-5xl md:text-6xl font-extrabold uppercase tracking-tighter">
              Women's<br />Collection
            </h1>
            <p className="text-gray-100 mt-4 text-lg max-w-md">
              Discover the latest trends in women's fashion and accessories.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="mb-6">
          ...
          {activeFilterCount === 0 && (
            <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">
              {womenProducts.length} premium items
            </p>
          )}
        </div>
